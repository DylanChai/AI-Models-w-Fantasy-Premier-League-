...
def parse_data():
    ...
    print("Getting data")
    data = get_data()  # Downloads the full FPL JSON data
    
    print("Parsing summary data")
    parse_players(data["elements"], base_filename)  # Writes players_raw.csv
    
    # Extract expected points (xP) for each player
    xPoints = []
    for e in data["elements"]:
        xPoint = {'id': e['id'], 'xP': e['ep_this']}
